			...
			<tbody>
				...
				<c:forEach var="c" items="${sessionScope.requests}">
					<tr>
						...
						<td> ${c.complaint.employee.name}</td>
						<td> ${c.complaint.consumer.address.city}</td>
						<td>${c.date}</td>
						<td>${c.stime}</td>
						<td>${c.etime}</td>
						<td>${c.status}</td>
					<td><a	href='<spring:url value="/Operator/togglelinecut?rid=${c.id}"/>'>
							<button	type="button" class="btn btn-danger">
							<i class="fa fa-tags">Power Cut</i>
							</button></a>
						<a	href='<spring:url value="/Operator/togglelinecut?rid=${c.id}"/>'>
						<button	type="button" class="btn btn-primary">
							<i class="fa fa-tags">Power on</i>
							</button></a>
					</td>
					</tr>
					</c:forEach>
